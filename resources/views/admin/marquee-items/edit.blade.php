...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title mb-4">Edit Marquee Item</h4>
                    <form method="POST" action="{{ route('admin.marquee-items.update', $marqueeItem->id) }}"
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" class="form-control" id="content" name="content"
                                value="{{ $marqueeItem->content}}" required>
                            @error('content')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select id="status2" name="status" class="form-control" required>
                                <option value="1" @if ($marqueeItem->status == 1) @selected(true) @endif>Active</option>
                                <option value="0"  @if ($marqueeItem->status == 0) @selected(true) @endif>Inactive</option>
                            </select>
                            @error('status')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
