        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('admin.testimonials.store') }}" class="row"
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select id="rating" name="rating" class="form-control" required>
                                @foreach ($ratingArray as $rating)
                                    <option value="{{$rating}}" @if ($rating == 5) @selected(true) @endif>{{$rating}}</option>
                                @endforeach
                            </select>
                            @error('status')
                                <div class="text-danger">{{ $message }}</div>
                        ...
                        <div class="col-md-12">
                            <div class="form-group mb-3">
                                ...
                                <textarea name="review" class="form-control" id="review" rows="4" placeholder="">{{ old('review') }}</textarea>
                                @error('review')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
